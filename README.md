...
goagent is a small, vendor‑agnostic CLI that talks to any OpenAI‑compatible Chat Completions API, executes a minimal allowlist of local tools (argv only, no shell), and prints the model’s final answer. Use it with hosted APIs or a local endpoint such as `http://localhost:1234/v1` when prototyping agents with strict, auditable tool execution.
...
### Quick start
Build the CLI and point it to a reachable OpenAI‑compatible API (local or hosted):
```bash
