...
>>>>>>> feat(cli): green test suite on main by porting missing helpers, removing duplicate ToolSpec, and implementing minimal pre-stage built-ins
...
=======
            ...
            if err := json.Unmarshal([]byte(tc.Function.Arguments), &args); err != nil {
                out = append(out, oai.Message{Role: oai.RoleTool, Name: name, ToolCallID: tc.ID, Content: sanitizeToolContent(nil, fmt.Errorf("invalid args: %v", err))})
                continue
            }
