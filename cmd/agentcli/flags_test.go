...
func TestResolveDeveloperMessages_Order(t *testing.T) {
    ...
    if err := os.WriteFile(f1, []byte("file-1"), 0o644); err != nil { t.Fatalf("write f1: %v", err) }
    if err := os.WriteFile(f2, []byte("file-2"), 0o644); err != nil { t.Fatalf("write f2: %v", err) }
...
func TestResolveDeveloperMessages_STDIN(t *testing.T) {
    ...
    if _, err := pw.Write([]byte("dev-stdin")); err != nil { t.Fatalf("write: %v", err) }
    _ = pw.Close()
    os.Stdin = pr
